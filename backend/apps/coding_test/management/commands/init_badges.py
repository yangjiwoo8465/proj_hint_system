"""
뱃지 초기 데이터 생성 management command
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for badge_data in BADGES:
            ...
            if created:
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f'[Created] {badge.name} ({badge.badge_type})'))
            else:
                updated_count += 1
                self.stdout.write(f'[Updated] {badge.name} ({badge.badge_type})')

        self.stdout.write(self.style.SUCCESS(f'\n총 {len(BADGES)}개 뱃지 처리 완료'))
        self.stdout.write(self.style.SUCCESS(f'생성: {created_count}개, 업데이트: {updated_count}개'))
